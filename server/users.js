...
let updateById = (req, res, next) => {
  ...
  let sql = "UPDATE user SET name='" + name + "', bio='" + bio + "', website='" + website + "', email='" + email + "' WHERE id=" + id + " RETURNING id; "
...
let submitNew = (req, res, next) => {
  ...
  let sql = "INSERT INTO users (id, email) VALUES ('" + name + "', '" + email + "') ON DUPLICATE KEY UPDATE 'email' = '" + email +"';";
 console.log(sql);
  db.query(sql)
    .then(item => res.json())
    .catch(next);
